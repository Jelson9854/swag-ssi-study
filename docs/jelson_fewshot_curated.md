# SCORE — Jelson 서브타입 few-shot 예제 (curated)

> 분류기 few-shot용. 예제는 **짧고 명확하게 정제·합성**한 것(Sang coded data의 패턴 기반, 원문 verbatim 아님). 각 서브타입 3개, 다양성 유지. 서브타입 = 팀의 working 코딩.
> 핵심 구분: **Translating** = 학생 *자기 글*을 잇기/다듬기 / **Reviewing** = 학생이 쓴 *기존 텍스트* 평가·교정 / **All** = ChatGPT가 *직접 생성/재작성*.

---

## Planning (계획 — 쓰기 전/주변에서 정보·아이디어 얻기)

**Information Search** (사실·정보 검색)
...
**Finish Paragraph (given a paragraph)**
...
**Rewrite a Human-written Sentence**
...
## All (ChatGPT가 직접 생성/재작성 — 전체 또는 일부)

**Write the Essay (full)**
...
**Write a Portion (paragraph / section)**
...
**Rewrite ChatGPT's Text (length / structure / detail)**
